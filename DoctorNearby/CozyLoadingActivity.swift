...
class CozyLoadingActivity: UIView {
    ...
    */
    ...
    convenience init(text: String, sender: UIViewController, disableUI: Bool) {
        let _ = UIScreen.mainScreen().bounds.size.width

        let width = sender.view.frame.width / 2
        let height = width / 4
        ...
        textLabel.font = UIFont(name: CLAFontName, size: 20)
    ...
    func hideLoadingActivity(success success: Bool, animated: Bool) {
        ...
        icon = UILabel(frame: CGRect(x: 20, y: frame.height/2 - 20, width: 40, height: 40))
        icon.font = UIFont(name: CLAFontName, size: 40)
